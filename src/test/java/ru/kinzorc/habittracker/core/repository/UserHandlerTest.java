...
class UserHandlerTest {
    ...
    void testHandleRegisterUserSuccess() {
        try (MockedStatic<MenuUtils> mockedInputUtils = Mockito.mockStatic(MenuUtils.class)) {
            // Мокаем ввод данных пользователя
            mockedInputUtils.when(() -> MenuUtils.promptValidInputUserData(eq(UserData.USERNAME), anyString(), anyString()))
                    .thenReturn("Test User");
            mockedInputUtils.when(() -> MenuUtils.promptValidInputUserData(eq(UserData.EMAIL), anyString(), anyString()))
                    .thenReturn("test@example.com");
            mockedInputUtils.when(() -> MenuUtils.promptValidInputUserData(eq(UserData.PASSWORD), anyString(), anyString()))
    ...
    void testHandleLoginUserSuccess() {
        try (MockedStatic<MenuUtils> mockedInputUtils = Mockito.mockStatic(MenuUtils.class)) {
            ...
            mockedInputUtils.when(() -> MenuUtils.promptInput("  Email: ")).thenReturn("test@example.com");
            mockedInputUtils.when(() -> MenuUtils.promptInput("  Пароль: ")).thenReturn("password123");
    ...
    void testHandleLoginUserFailed() {
        try (MockedStatic<MenuUtils> mockedInputUtils = Mockito.mockStatic(MenuUtils.class)) {
            // Мокаем ввод email и пароля
            mockedInputUtils.when(() -> MenuUtils.promptInput("  Email: ")).thenReturn("test@example.com");
            mockedInputUtils.when(() -> MenuUtils.promptInput("  Пароль: ")).thenReturn("wrongpassword");
    ...
    void testHandleDeleteUserSuccess() {
        try (MockedStatic<MenuUtils> mockedInputUtils = Mockito.mockStatic(MenuUtils.class)) {
            // Мокаем ввод пароля
            mockedInputUtils.when(() -> MenuUtils.promptInput("Введите пароль для удаления аккаунта: ")).thenReturn("password123");
    ...
    void testHandleDeleteUserFailed() {
        try (MockedStatic<MenuUtils> mockedInputUtils = Mockito.mockStatic(MenuUtils.class)) {
            // Мокаем ввод пароля
            mockedInputUtils.when(() -> MenuUtils.promptInput("Введите пароль для удаления аккаунта: ")).thenReturn("wrongpassword");
